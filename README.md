...
- **Request Line**: contains the request method, the name of the requested resource, and the version of HTTP in use.
...
- **Whitespace**: a blank line indicating the end of the request.
...
- The **POST method** sends data to the specified resource, often changing or updating the server.
...
- The advantage to using curl is when no other GUI tools exist or are limited to send data to and from a server. 
...
- The `-X` **option** specifies that we are changing the request method, followed by the type of method that we want to use in our request.

12. Which `curl` option is used to set request headers?
- The `-H` **option** specifies that we are using a new header value.
...
15. Which response header sends a cookie to the client?
    ...
    Set-Cookie: cart=Bob
    ```
The **Set-Cookie: cart=Bob** response header sends Bob's cart cookie to the client.
...
16. Which request header will continue the client's session?
    ...
    Cookie: cart=Bob
    ```
The **Cookie: cart=Bob** request header will continue the client's session.
...
20. What kind of data is being sent from this request body? **Login credentials for Barbara.**
...
21. What is the response status code? **200 OK**

22. What web server is handling this HTTP response? **Apache**

23. Does this response have a user session associated to it? **Yes, a session ID with value of  5.**

24. What kind of content is likely to be in the [page content] response body? **Content-Type: text/html; charset=UTF-8**
...
**Containerization because replication of identical components allows you to serve more clients and provides identical backup components if one fails.**
...
- **Docker Compose is a tool that can deploy and configure multiple containers at once.**
